...
- 无可靠院系映射的学校专业参考记录已拆成共享参考证据；ISSUE-031 第四阶段进一步保存来源实体与映射状态，但仍不把候选映射提升为方向级事实。
- `0`与未知状态已在当前招生计划人数上解耦；其他工作区字段仍转入 ISSUE-031 后续阶段处理。
...
- CDP页面确认：计划行不再用最新参考线285覆盖335；无主分数院校显示“暂无”；当时招生人数0仍显示“未提供”，后续 ISSUE-031 第五阶段已修正为来源明确的0显示为0。
