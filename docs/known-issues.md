...
| ISSUE-032 | medium | open | 官方录取统计采集、同步、查询和独立 UI 尚未接入 |

**统计**：共 32 个 ISSUE，31 个 fixed，0 个 partial-fixed，0 个 in-progress，1 个 open。

**下一步优先级**：ISSUE-031 已按既定十阶段范围收尾；ISSUE-032 在取得可验证的官方录取统计样本后启动，不阻塞当前发布。
...
- **严重程度**：medium
- **状态**：open
- **描述**：系统当前只有复试分数线和参考线，没有官方录取最低分、平均分、最高分和录取人数的数据管道。
- **期望行为**：按 `docs/issue-031-admission-statistics-design.md` 独立采集、同步、查询和展示录取统计，完整保留官方原始证据。
- **约束**：不得写入 `score_lines.total` 或现有工作区分数线字段；没有可验证官方样本前不创建生产表。
- **验收入口**：先确认至少两个学校、两个年份、包含不同统计口径的官方样本，再按设计文档的测试契约实施。
