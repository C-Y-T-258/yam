...
| ISSUE-031 | medium | fixed | 来源映射、未知状态、计划年份边界和录取统计独立模型契约已完成 |
| ISSUE-032 | medium | open | 年度来源研究已完成；官方录取统计证据层、同步、查询和独立 UI 尚未接入 |
...
## ISSUE-031：来源映射、未知状态和录取统计尚未独立建模

- **严重程度**：medium
- **状态**：fixed
- **描述**：共享分数证据、版本化计划快照、稳定身份、来源映射、招生人数状态、未知值审计与展示、计划年份来源边界和录取统计独立模型契约均已完成；实际录取统计数据接入转入 ISSUE-032。
- **影响范围**：
  - 掌上考研来源院系已可审计映射到研招网院系或学校专业层，但业务投影仍保持学校专业参考层，下游统计必须避免按计划重复计数。
  - 当前考试科目、研究方向、学习方式和专项计划可能被误解为历史年份事实。
  - 部分工作区字段仍用 `0` 或空字符串表示未知状态；当前招生计划人数已拆为数值、`enrollment_count_status` 和 `enrollment_text`。
  - 考试科目、院系名称、学习方式等快照字段变化已不再改变 `plan_key`；但来源没有方向ID，方向改名仍需后续映射证据才能跨年归并。
...
- **第一阶段实施**：
  - 新增 `workspace_score_evidence`，学校、专业、年份级分数证据只物理保存一次，保留原始证据 JSON、来源记录数和选中来源院系。
  - 新增 `workspace_plan_score_evidence`，计划通过引用共享证据；`workspace_plan_years` 仅保留未来可精确映射到计划的直接年份事实。
  - normalized 模型升级为 v3，Rust 通过直接计划年份与共享证据引用的联合视图维持现有 DTO、筛选、分页和 UI 行为。
  - `085410` 真实源库到临时目标库验证：514个计划、668条共享证据、1624个引用、0条重复计划年份值；模型投影仍为1624条且状态为`ready`。
  - 同步门禁拒绝悬空、跨学校或跨专业的证据引用；专项测试验证两个计划共享一条证据且清理专业时引用与证据同步删除。
- **第二阶段实施**：
  - 新增 `workspace_plan_snapshots`，按 `plan_key + observed_at` 保存当前招生目录观察值；重复同步同一观察时间执行 upsert，正常定向重同步保留历史快照。
  - `catalog_year` 可空，并用 `catalog_year_status=provided/unknown` 明确来源是否提供目录年份；现有研招网来源未提供可靠年份时不再借用最新分数年份。
  - normalized 模型升级为 v4；Rust DTO、页面与导出分开返回目录年份状态、计划快照时间和最新分数年份，兼容字段在未知时保持 `0`，页面显示“未知”。
  - `085410` 真实只读源库到临时目标库验证：217所学校、514个计划、514个当前快照、1624条分数证据投影；全部快照为明确 `unknown`，重复同步后仍为514条，模型状态为 `v4/ready`。
  - Python、Rust 和前端专项测试覆盖未知目录年份、快照去重、独立最新分数年份及新增导出列。
- **第三阶段实施**：
  - `department_key v2` 优先使用学校、专业和源院系ID；源ID缺失时才回退院系名。`plan_key v2` 只使用稳定院系键和研究方向，移除院系名、考试科目、学习/考试方式和专项计划等版本字段。
  - 快照新增源院系ID、院系名和身份版本；定向重同步会在清理当前投影前迁移 v1 快照，同一计划、同一观察时间的重复项合并，其余历史观察保留。
  - normalized 模型升级为 v5；Rust 与 Python schema 均能兼容升级 v4 快照表，专项测试证明版本字段变化不换键且旧快照可迁移。
  - 四专业真实源库全量验证：2961个计划生成2961个唯一 v2 计划键、1217个稳定院系实体，0碰撞、0快照孤儿；四个专业均为 `v5/ready`。`085410` 的514个 v4 快照迁移后不丢不增，重复同步仍为514。
  - 研招网没有独立方向ID，因此方向文本仍是区分同院系多计划的必要字段；禁止用模糊文本自动合并，留待来源实体映射阶段处理。
- **第四阶段实施**：
  - 新增 `workspace_source_entities` 保存掌上考研分数来源实体；`workspace_score_evidence.source_entity_key` 让每条共享证据可回溯到来源院系ID/名称、raw code 和原始 JSON。
  - 新增 `workspace_entity_mappings` 保存来源实体到研招网院系或学校专业层的映射状态：`candidate`、`ambiguous`、`unmapped`，并记录规则与置信度。
  - 只允许唯一规范化名称匹配成为 `candidate`，同名多候选为 `ambiguous`，无唯一候选为 `unmapped`；不做模糊匹配，不把映射候选提升为方向级分数事实。
  - normalized 模型升级为 v6；真实四专业同步验证：2714条共享证据全部有来源实体，1144个来源实体全部有映射记录，其中212个院系候选、932个学校专业层未映射、0个缺映射；业务分数投影仍为9135条。
- **第五至第九阶段实施**：
  - 当前招生人数拆为数值、状态和原文；来源明确的0保留，未知0不再混淆。
  - 新增可重复只读审计，真实源库和桌面库确认分数、单科、目录年和招生人数的哨兵边界。
  - 工作区、比较、收藏和导出统一未知值语义；未知数值导出为空并保留状态列。
  - 真实源表没有目录年份列，2961个快照全部为 `NULL/unknown`；测试证明未来可靠年份可进入 `provided`，禁止从分数年份推断。
- **第十阶段实施**：
  - 固定 `workspace_admission_statistics` 最小模型、查询接口、约束和测试契约。
  - 录取统计禁止写回分数线模型；真实采集、同步和 UI 接入转入 ISSUE-032。
- **不属于当前发布阻塞项**：现有 UI 已明确展示分数粒度、年份、请求状态和 fallback 类型，ISSUE-030 的用户可见错误已闭环。

---

## ISSUE-032：官方录取统计尚未接入

- **严重程度**：medium
- **状态**：open
- **描述**：系统当前只有未严格分型的分数参考记录，没有官方录取最低分、平均分、最高分、复试成绩、最终成绩和录取人数的数据管道。
- **前置研究**：`docs/research/year-source/` 已确认当前产品目录覆盖 0%、严格分型分数覆盖 0%、录取统计覆盖 0%；深圳大学样本证明官方录取人数与初试统计可人工复核，但复试成绩和最终总成绩仍未取得可复核样本。
- **期望行为**：按 `docs/issue-031-admission-statistics-design.md` 和 `docs/research/year-source/evidence/admission-cycle-contract.json` 独立采集、同步、查询和展示录取统计，完整保留官方原始证据。
- **约束**：不得写入 `score_lines.total` 或现有工作区分数线字段；先做小样本影子数据集和证据层，不直接创建生产表或 UI。
- **验收入口**：至少完成学校官方样本的原文留证、`source_year/source_year_kind`、实体映射粒度、版本链和隐私扫描，再按设计文档的测试契约实施正式接入。
