...
- **严重程度**：medium
- **状态**：in-progress（共享分数证据、当前计划快照和来源范围内稳定身份已完成，其余映射、未知状态和统计边界待处理）
- **描述**：ISSUE-030 已阻止错误分数组合和误导性展示，ISSUE-031 前三阶段已独立建模共享分数证据、当前计划快照和来源范围内稳定身份；仍待处理的通用边界包括不同数据源实体缺少显式映射证据、未知/未公布/失败/fallback 尚未在所有表中使用独立状态，以及可靠历史计划事实缺少来源。
- **影响范围**：
  - 无可靠院系映射的学校专业参考证据已独立存储，但仍通过引用投影到多个计划，下游统计必须避免按计划重复计数。
