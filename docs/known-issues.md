...
| ISSUE-031 | medium | in-progress | 共享证据、计划快照、稳定身份、来源映射和招生人数状态已完成；剩余未知状态边界与录取统计待处理 |

**统计**：共 31 个 ISSUE，30 个 fixed，0 个 partial-fixed，1 个 in-progress，0 个 open。

**下一步优先级**：ISSUE-030 已完成用户可见闭环；ISSUE-031 接下来处理剩余未知状态边界和录取统计，不阻塞当前发布。
...
- **严重程度**：medium
- **状态**：in-progress（共享分数证据、当前计划快照、稳定身份、来源映射审计层和招生人数状态已完成，剩余未知状态和统计边界待处理）
- **描述**：ISSUE-030 已阻止错误分数组合和误导性展示，ISSUE-031 前五阶段已独立建模共享分数证据、当前计划快照、来源范围内稳定身份、来源实体映射审计层和当前计划招生人数状态；仍待处理的通用边界包括其他未知/未公布/失败/fallback 字段尚未全部使用独立状态，以及可靠录取统计模型缺失。
- **影响范围**：
