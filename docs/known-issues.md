...
| ISSUE-031 | medium | in-progress | 来源实体映射、时间版本、未知状态和计划稳定键仍需独立建模 |
...
- **严重程度**：medium
- **状态**：in-progress（共享学校专业分数证据与当前计划快照已独立建模，其余映射、身份和统计边界待处理）
- **描述**：ISSUE-030 已阻止错误分数组合和误导性展示，ISSUE-031 前两阶段已独立建模共享分数证据和当前计划快照；仍待处理的通用边界包括不同数据源实体缺少显式映射证据、未知/未公布/失败/fallback 尚未在所有表中使用独立状态、可靠历史计划事实缺少来源，以及 `plan_key` 仍包含可能随年度变化的展示字段。
- **影响范围**：
