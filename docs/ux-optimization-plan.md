...
- **二次优化（2026-07-24）**：用户反馈图表"像被拍扁拍大了一样"。进一步调整：
  - 画布比例从约 2:1 改为 21:13（约 1.62:1），折线起伏更明显；
  - 限制单张卡片最大宽度 `max-w-2xl`（672px），避免在宽屏上被无限拉宽；
  - 折线粗细 2.5px→2px，数据点默认 r=5→r=4，hover r=7→r=6.5；
  - 坐标轴/网格线减淡（`#e2e8f0` / `#f1f5f9`），Y 轴刻度从 4 档增至 5 档；
  - 图表容器改为响应式网格 `grid grid-cols-1 xl:grid-cols-2 gap-6`，大屏并排展示两张图；
  - 卡片 padding p-5→p-4，增加 `shadow-sm` 提升层次。
- **验证结果**：`npm run build` 通过，`cargo check` 通过；CDP 截图确认最低分趋势图 348→330→330→264 下降清晰，招生人数趋势图显示「无变化」，两张图在 1920×1200 视口下并排展示，不再"拍扁"。
- **涉及文件**：`yam-desktop/src/pages/WorkspacePage.tsx`
